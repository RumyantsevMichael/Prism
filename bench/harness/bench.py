...
def ask_product_owner(spec_text, message, po_model, log_dir, tag):
    ...
    (Path(log_dir) / f"{tag}.po.json").write_text(stdout or "")
...
def run_phase(
    ...
    for exchange in range(max_exchanges + 1):
        ...
        (log_dir / f"{phase}.{exchange}.stdout.json").write_text(stdout or "")
        if stderr:
            (log_dir / f"{phase}.{exchange}.stderr.txt").write_text(stderr)
...
def run_stage_phases(args, arm_info, task, stage_index, wdir, log_dir):
    """All agent sessions for one stage of one rep. Returns phase records."""
    stage = task["stages"][stage_index]
    prism_arm = arm_info["plugin_dir"] is not None
    slug = stage["name"]
    ...
    phases = [
        run_phase(
            ...
            plugin_dir=arm_info["plugin_dir"],
...
def cmd_run(args):
    ...
    for name, ref in arms:
        ...
        resolved[name] = {"ref": ref, "plugin_dir": str(plugin_dir) if plugin_dir else None, "sha": sha}
        if worktree:
                    ...
                    for rep in range(1, args.reps + 1):
                        ...
                        for stage_index, stage in enumerate(stages_to_run):
                            ...
                            if args.mock:
                                ...
                                phases = run_stage_phases(args, arm_info, task, stage_index, wdir, log_dir)
