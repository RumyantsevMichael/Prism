...
Run it in a new session, because its independence from the design session is exactly what makes it a fair validator.
...
The nuance here: if `/plan` or `/design` already ran in this session, stop and have the user run `/implement` in a fresh one.
The cold read *is* the validation pass.
...
Run it yourself where you can (load `verify`).
...
2. **Last track? Graduate, then delete the initiative.** If this is the final track, run the graduate-before-delete gate.
   ...
   **Keep the roadmap's state column honest.** On the **last** track, flip the initiative's roadmap node (default `/docs/roadmap.md`) `→ shipped` and remove its plan deep-link.
