...
There is **no technical design, no contracts, no feature files, no build plan** here; those belong to `/design`.
...
- **Which strategy pillar does it serve?** An idea that serves none is a flag, not necessarily a kill — surface it.
...
- **Several distinct decisions → several ADRs.** An idea that fragments into a cluster of decisions is an **initiative** → recommend `/plan` (which decomposes it into tracks and handles its roadmap node — `/ideate` does not).
